...
export default function UserSearchModal({ isOpen, onClose, onSelectUser }) {
  ...
  const searchUsers = async () => {
    setLoading(true)
    // Veritabanımızda email saklamadığımız için Ad Soyad (full_name) ile arama yapıyoruz
            ...
            <div className="max-h-60 overflow-y-auto p-2 custom-scrollbar">
              ...
              {results.map(user => (
                <div key={user.id} onClick={() => onSelectUser(user)}
                  ...
                  <button className="ml-auto bg-blue-600 text-xs px-3 py-1 rounded-full text-white hover:bg-blue-500">Seç</button>
                </div>
              ))}
            </div>
          </motion.div>
