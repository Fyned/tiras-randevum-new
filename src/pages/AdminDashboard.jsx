...
export default function AdminDashboard() {
  ...
  const checkUserRole = async () => {
    ...
    if (profile) {
        ...
        else { alert("Yetkisiz Giriş"); navigate('/'); }
    } else { navigate('/') }
    setLoading(false)
  }
  ...
  const assignManager = async (user) => {
    if(!confirm(`${user.full_name} bu dükkanın yöneticisi olsun mu?`)) return;
    ...
    if(!roleError) { alert('✅ Yönetici atandı!'); setIsUserSearchOpen(false); fetchShops() } else { alert(roleError.message) }
  }
