                    ...
                    {appointments.map(app => (
                        <div key={app.id} className={`p-4 rounded-2xl border flex flex-col md:flex-row justify-between items-center gap-4 ${app.status === 'pending' ? 'bg-yellow-500/10 border-yellow-500/30' : 'bg-green-500/10 border-green-500/30'}`}>
                            <div>
                                ...
                                {/* HATA DÜZELTME: Eğer service veya barber null ise 'Silinmiş' yaz */}
                                <div className="text-sm text-gray-400 mt-1">
                                    {app.services?.name || 'Hizmet Silinmiş'} - {app.barbers?.full_name || 'Berber Silinmiş'}
                                </div>
                            </div>
                        ...
                        <div key={app.id} className="p-4 bg-black/20 rounded-2xl border border-white/5 flex justify-between items-center opacity-70 hover:opacity-100 transition-opacity">
                            <div>
                                ...
                                <div className="text-xs text-gray-500">
                                    {new Date(app.start_time).toLocaleDateString()} - {app.services?.name || 'Hizmet Silinmiş'}
                                </div>
                            </div>
                    ...
                    {days.map((day, index) => {
                        ...
                        return (
                            <div key={index} className="flex items-center justify-between bg-black/20 p-4 rounded-2xl border border-white/5">
                                ...
                                <div className="flex items-center gap-3"><label className="flex items-center cursor-pointer"><input type="checkbox" defaultChecked={schedule.is_active} id={`active-${index}`} className="mr-2 accent-green-500"/><span className="text-xs">Açık</span></label><button onClick={() => {const start = document.getElementById(`start-${index}`).value; const end = document.getElementById(`end-${index}`).value; const active = document.getElementById(`active-${index}`).checked; saveSchedule(index, start, end, active, barbers[0].id)}} className="bg-blue-600 px-3 py-1 rounded-lg text-xs text-white">Kaydet</button></div>
                            </div>
                        )
                    })}
                </div>
                ...
                <div className="bg-white/5 backdrop-blur-md border border-white/10 p-6 rounded-[30px]">
                    ...
                    <div className="space-y-3 mb-6 max-h-[400px] overflow-y-auto custom-scrollbar">
                        ...
                        <div key={b.id} className="bg-black/40 p-3 rounded-2xl flex justify-between items-center group">
                            <div className="flex items-center gap-4"><label className="relative w-12 h-12 rounded-full overflow-hidden cursor-pointer group/avatar border border-white/10 bg-gray-700">{b.avatar_url ? (<img src={b.avatar_url} className="w-full h-full object-cover" alt={b.full_name} />) : (<div className="w-full h-full bg-gradient-to-tr from-blue-600 to-indigo-600 flex items-center justify-center text-sm">👨‍id</div>)}<div className="absolute inset-0 bg-black/60 flex items-center justify-center opacity-0 group-hover/avatar:opacity-100 transition-opacity"><span className="text-xs">📷</span></div><input type="file" accept="image/*" className="hidden" onChange={(e) => updateBarberAvatar(e, b.id)} disabled={uploading} /></label><span className="font-medium text-gray-200">{b.full_name}</span></div>
                    ...
                    <form onSubmit={addBarber} className="flex gap-2"><input value={barberName} onChange={e=>setBarberName(e.target.value)} placeholder="Yeni Berber Adı" className="flex-1 bg-white/5 border border-white/10 rounded-xl p-3 text-sm focus:outline-none focus:border-blue-500 transition-colors" required/><button className="bg-blue-600 hover:bg-blue-500 text-white px-4 rounded-xl shadow-lg font-bold">+</button></form>
                </div>
                <div className="bg-white/5 backdrop-blur-md border border-white/10 p-6 rounded-[30px]">
                    <h3 className="text-xl font-bold mb-6 text-emerald-300">Hizmet Menüsü</h3>
                    <div className="space-y-3 mb-6 max-h-[400px] overflow-y-auto custom-scrollbar">
                        {services.map(s => (<div key={s.id} className="bg-black/40 p-3 rounded-2xl flex justify-between items-center group"><div><div className="text-sm">{s.name}</div><div className="text-xs text-gray-500">{s.duration_min} dk</div></div><div className="flex items-center gap-3"><span className="text-emerald-400 font-bold">{s.price}₺</span><button onClick={() => deleteService(s.id)} className="text-red-500 opacity-0 group-hover:opacity-100 transition-opacity p-2 hover:bg-white/10 rounded-lg">🗑️</button></div></div>))}
                    </div>
                    <form onSubmit={addService} className="space-y-3"><input value={serviceName} onChange={e=>setServiceName(e.target.value)} placeholder="Hizmet Adı" className="w-full bg-white/5 border border-white/10 rounded-xl p-3 text-sm focus:outline-none focus:border-emerald-500" required/><div className="flex gap-2"><input type="number" value={servicePrice} onChange={e=>setServicePrice(e.target.value)} placeholder="Fiyat" className="w-1/2 bg-white/5 border border-white/10 rounded-xl p-3 text-sm focus:outline-none focus:border-emerald-500" required/><input type="number" value={serviceDuration} onChange={e=>setServiceDuration(e.target.value)} placeholder="Süre (dk)" className="w-1/2 bg-white/5 border border-white/10 rounded-xl p-3 text-sm focus:outline-none focus:border-emerald-500" required/></div><button className="w-full bg-emerald-600 hover:bg-emerald-500 text-white py-3 rounded-xl font-bold shadow-lg">Ekle</button></form>
                </div>
            </div>
        )}
            ...
            <div className="space-y-8">
                ...
                <div className="bg-white/5 backdrop-blur-md border border-white/10 p-6 rounded-[30px]">
                    ...
                    <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-2">
                        {portfolio.map(item => (<div key={item.id} className="relative aspect-square group rounded-xl overflow-hidden bg-black/40">{item.media_type === 'video' ? (<video src={item.media_url} className="w-full h-full object-cover" />) : (<img src={item.media_url} className="w-full h-full object-cover" loading="lazy"/>)}<div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center"><button onClick={() => deleteFromGallery(item.id)} className="bg-red-600 text-white p-2 rounded-full hover:scale-110 transition-transform">🗑️</button></div></div>))}
                    </div>
                </div>
            </div>
        )}
