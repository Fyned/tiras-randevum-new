...
export default function ShopOwnerDashboard() {
  ...
  // SEKMELER: management (Ekip/Hizmet), profile (Galeri), schedule (Saatler), appointments (Randevular), history (Geçmiş)
  ...
  const handleImageUpload = async (file, bucket) => {
    ...
    } catch (error) { alert('Yükleme hatası: ' + error.message); return null }
  }
    ...
    <div className="min-h-screen pt-36 pb-20 px-4 bg-[#0F172A] text-white font-sans">
      ...
      <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} className="max-w-6xl mx-auto">
        ...
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 p-8 rounded-[40px] mb-8 flex flex-col md:flex-row items-center gap-6">
          <div className="relative group">
            ...
            <label className="absolute bottom-0 right-0 bg-blue-600 p-2 rounded-full cursor-pointer hover:scale-110 transition-transform shadow-lg">
                ...
                <input type="file" accept="image/*" onChange={updateShopAvatar} className="hidden" disabled={uploading} />
            </label>
          </div>
        ...
        <div className="flex flex-wrap gap-2 md:gap-4 mb-6 border-b border-white/10 pb-1 overflow-x-auto">
            ...
            <button onClick={()=>setActiveTab('profile')} className={`pb-3 px-4 font-medium transition-colors ${activeTab==='profile' ? 'text-purple-400 border-b-2 border-purple-400' : 'text-gray-500 hover:text-white'}`}>🖼️ Profil</button>
        </div>
                        ...
                        <div key={app.id} className={`p-4 rounded-2xl border flex flex-col md:flex-row justify-between items-center gap-4 ${app.status === 'pending' ? 'bg-yellow-500/10 border-yellow-500/30' : 'bg-green-500/10 border-green-500/30'}`}>
                            <div>
                                <div className="flex items-center gap-3 mb-1">
                                    ...
                                    {app.status==='pending' && <span className="bg-yellow-500 text-black text-[10px] font-bold px-2 py-0.5 rounded">ONAY BEKLİYOR</span>}
                                </div>
                                <div className="text-white font-medium flex items-center gap-2">
                                   {app.customer_name} <span className="text-gray-500 text-sm">({app.customer_phone})</span>
                                   {app.created_by_user && (<button onClick={() => setSelectedCustomerUserId(app.created_by_user)} className="ml-2 text-xs bg-blue-500/20 text-blue-300 px-2 py-0.5 rounded hover:bg-blue-500 hover:text-white transition-colors">Profili Gör</button>)}
                                </div>
                            ...
                            <div key={index} className="flex items-center justify-between bg-black/20 p-4 rounded-2xl border border-white/5">
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <button onClick={() => {
                                        ...
                                        }} className="bg-blue-600 px-3 py-1 rounded-lg text-xs text-white">Kaydet</button>
                                </div>
                            </div>
                        )
                    })}
                        ...
                        <div key={b.id} className="bg-black/40 p-3 rounded-2xl flex justify-between items-center group">
                            <div className="flex items-center gap-4">
                                <label className="relative w-12 h-12 rounded-full overflow-hidden cursor-pointer group/avatar border border-white/10 bg-gray-700">
                                    {b.avatar_url ? (<img src={b.avatar_url} className="w-full h-full object-cover" alt={b.full_name} />) : (<div className="w-full h-full bg-gradient-to-tr from-blue-600 to-indigo-600 flex items-center justify-center text-sm">👨‍id</div>)}
                                    ...
                                    <input type="file" accept="image/*" className="hidden" onChange={(e) => updateBarberAvatar(e, b.id)} disabled={uploading} />
                                </label>
                ...
                <div className="bg-white/5 backdrop-blur-md border border-white/10 p-6 rounded-[30px]">
                    ...
                    <div className="space-y-3 mb-6 max-h-[400px] overflow-y-auto custom-scrollbar">
                        ...
                        <div key={s.id} className="bg-black/40 p-3 rounded-2xl flex justify-between items-center group">
                            <div><div className="text-sm">{s.name}</div><div className="text-xs text-gray-500">{s.duration_min} dk</div></div>
                            <div className="flex items-center gap-3"><span className="text-emerald-400 font-bold">{s.price}₺</span><button onClick={() => deleteService(s.id)} className="text-red-500 opacity-0 group-hover:opacity-100 transition-opacity p-2 hover:bg-white/10 rounded-lg">🗑️</button></div>
                        </div>
                        ))}
                    </div>
